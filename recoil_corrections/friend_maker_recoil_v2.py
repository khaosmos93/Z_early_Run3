...
def calculateMET(rdf, corrected):
    rdf = rdf.Define("metPx"+corrected, "-lepPt*cos(lepPhi) - uP1"+corrected+"*cos(genbosonphi) + uP2"+corrected+"*sin(genbosonphi)") # TODO check if name already used
    rdf = rdf.Define("metPy"+corrected, "-lepPt*sin(lepPhi) - uP1"+corrected+"*sin(genbosonphi) - uP2"+corrected+"*cos(genbosonphi)")

    rdf = rdf.Define("met"+corrected, "sqrt(metPx"+corrected+"*metPx"+corrected+" + metPy"+corrected+"*metPy"+corrected+")")
    rdf = rdf.Define("metphi"+corrected, "atan2(metPy"+corrected+", metPx"+corrected+")")
...
def main():
    ...
    dataWorkspace = load_fit_results(args.wsdir + ws_dict[args.corr]['data'], nBins) 
    mcWorkspace = load_fit_results(args.wsdir + ws_dict[args.corr]['mc'], nBins)
    toCorrWorkspace = load_fit_results(args.wsdir + ws_dict[args.corr]['mc'], nBins)
    ...
    outdir = '/ceph/jdriesch/CROWN_samples/' + period + '/friends/' + year + '/' + process + "/" + args.finalstate + "/"
    ...
    while os.path.exists(inpath + "_"+str(i)+".root"):
        ...
        if not "DY" in process:
            rdf = rdf.Define("uP1"+corrected, "uP1")
            rdf = rdf.Define("uP2"+corrected, "uP2")
            rdf = rdf.Define("met"+corrected, "met")
            rdf = rdf.Define("metphi"+corrected, "metphi")

            if not os.path.exists(outfile) or args.overwrite:
                rdf.Snapshot("ntuple", outfile, ["uP1"+corrected, "uP2"+corrected, "met"+corrected, "metphi"+corrected])
        ...
        tordf = {"uP1"+corrected: fromrdf["uP1"], "uP2"+corrected: fromrdf["uP2"], "lepPt": fromrdf["lepPt"], "lepPhi": fromrdf["lepPhi"], "genbosonphi": fromrdf["genbosonphi"]}
        corr = {}
        ...
        rdf_tosave = calculateMET(rdf_tosave, corrected)     

        if not os.path.exists(outfile) or args.overwrite:
            rdf_tosave.Snapshot("ntuple", outfile, ["uP1"+corrected, "uP2"+corrected, "met"+corrected, "metphi"+corrected])
